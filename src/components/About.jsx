...
export default function About() {
  ...
  const Content = (
    <div className="mx-auto max-w-5xl rounded-2xl border border-slate-200 bg-white px-6 py-10 shadow-sm md:px-10">
      <div className="mb-8 flex flex-col items-center text-center">
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <div className="text-left">
            <h2 className="text-xs font-semibold uppercase tracking-[0.2em] text-sky-600">
              About AMVA
            </h2>
      ...
      <p className="mx-auto mb-8 max-w-3xl text-sm leading-relaxed text-slate-600 md:text-base">
        AMVA Global Solution Pvt Ltd partners with technology, GCC and
        enterprise clients to build and manage their presence in India. We
        combine structured processes, local expertise and thoughtful service
        design to ensure your teams operate with confidence and clarity from
        day one.
      </p>
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <div>
          ...
          <p className="mb-5 rounded-xl bg-slate-50 p-4 text-sm leading-relaxed text-slate-600">
            To be the most dependable partner for global organizations building
            high-performing teams and infrastructure in India.
          </p>
          ...
          <p className="rounded-xl bg-slate-50 p-4 text-sm leading-relaxed text-slate-600">
            To simplify global expansion with structured, compliant and scalable
            solutions in hiring, HR, workspace and IT—delivered through one
          ...
          <ul className="space-y-3">
            {coreValues.map((v) => (
              <li key={v.name} className="flex items-start gap-2">
                <span className="mt-[3px] h-1.5 w-1.5 rounded-full bg-sky-600" />
                <div>
      ...
      <div className="mt-10">
        <h3 className="mb-4 text-center text-lg font-semibold text-slate-900">
          Why organizations choose AMVA
        </h3>
        <div className="grid gap-6 md:grid-cols-3">
          {differentiatorCards.map((card) => (
            <div
              key={card.title}
              className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-5 text-center shadow-sm"
            >
              <img
                src={card.image}
                alt={card.title}
    ...
    <section id="about" className="bg-slate-50 py-16">
      ...
      >
        <motion.div
          ...
          className="mx-auto max-w-5xl rounded-2xl border border-slate-200 bg-white px-6 py-10 shadow-sm md:px-10"
        >
          {Content}
        </motion.div>
      </motion.div>
